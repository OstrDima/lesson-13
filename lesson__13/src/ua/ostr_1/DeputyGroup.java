...
public class DeputyGroup {
	...
	public void addDeputy() {
		...
		System.out.println("Депутат " + deputy.toString() + " успішно доданий у фракцію!");
	}
		...
		if (typeCorrect) {
			...
			while (iterator.hasNext()) {
				...
				if (nextDeputy.getSurname().equalsIgnoreCase(surname) && nextDeputy.getName().equalsIgnoreCase(name)) {
					iterator.remove();
					System.out.println("Депутат " + nextDeputy.toString() + " успішно видалено з фракції!");
				}
			}

		} else {
			System.out.println("Введеного депутата немає в цій фракції!");
		}
	}

	public void getBribeTakers() {
		System.out.println("Депутати фракції, які беруть хабарі:");
	...
	public void getLargestBribeTaker() {
		...
		if (deputyIndex >= 0) {
			...
			System.out.println("У цій партії хабарників немає!");
		}
	}

	public void getAllDeputies() {
		System.out.println("Депутати, які перебувають у цій фракції:");
		for (Deputy deputy : deputyGroupArray) {
			System.out.println(deputy.toString());
		}
	}
	...
	public void clearDeputyGroup() {
		...
		System.out.println("Усіх депутатів було успішно видалено з фракції!");
	}
